      ...
      <div className="container mx-auto px-4">
        <div className="flex h-20 items-center justify-between">
          ...
          <div className="flex flex-1 flex-col items-start gap-1">
            ...
            >
              <div className="relative h-9 w-9 rounded-full overflow-hidden border-2 border-[#29abe2] bg-[hsl(var(--color-background))] shadow-sm">
                <img 
                  ...
                  <div className="fixed md:absolute left-4 right-4 md:left-auto md:right-0 top-[22px] md:top-1/2 md:-translate-y-1/2 z-50 animate-in fade-in slide-in-from-right-4">
                    <div className="relative w-full md:w-96">
                      ...
                      {searchResults.length > 0 && (
                        <div className="absolute top-full left-0 right-0 mt-2 bg-[hsl(var(--color-background))] border border-[hsl(var(--color-border))] rounded-xl shadow-xl overflow-hidden max-h-[60vh] overflow-y-auto">
                          <ul className="py-2">
                            {searchResults.map((result, index) => (
                              <li key={result.tool.id}>
                                <button onClick={() => navigateToTool(result.tool.slug)} onMouseEnter={() => setSelectedIndex(index)} className={`w-full px-4 py-2.5 text-left flex items-center gap-3 ${index === selectedIndex ? 'bg-[hsl(var(--color-primary))/0.1] text-[hsl(var(--color-primary))]' : 'hover:bg-[hsl(var(--color-muted))]'}`}>
